...
bool adim;                      //!< Whether to read the adim. var. or not
param pdim;                        //!< Model dimensional parameters
  ...
  if (cfg->exists("model"))
    {
      ...
      std::cout << "Model scales: " << std::endl;
      pdim["T0"] = cfg->lookup("model.T0");
      std::cout << "T0 = " << pdim["T0"] << std::endl;
      pdim["DeltaT"] = cfg->lookup("model.DeltaT");
      std::cout << "DeltaT = " << pdim["DeltaT"] << std::endl;
      pdim["L"] = cfg->lookup("model.L");
      std::cout << "L = " << pdim["L"] << std::endl;
      pdim["Ly"] = cfg->lookup("model.Ly");
      std::cout << "Ly = " << pdim["Ly"] << std::endl;
      pdim["Hs"] = cfg->lookup("model.Hs");
      std::cout << "Hs = " << pdim["Hs"] << std::endl;
      pdim["c0"] = cfg->lookup("model.c0");
      std::cout << "c0 = " << pdim["c0"] << std::endl;
      pdim["tau0"] = cfg->lookup("model.tau0");
      std::cout << "tau0 = " << pdim["tau0"] << std::endl;

      adim = cfg->lookup("model.adim");
      if (adim) {
	std::cout << "Reading model adimensional parameters..." << std::endl;
	p["xs0"] = cfg->lookup("model.xs0");
	p["alpha"] = cfg->lookup("model.alpha");
	p["deltas"] = cfg->lookup("model.deltas");
	p["eta1"] = cfg->lookup("model.eta1");
	p["eta2"] = cfg->lookup("model.eta2");
	p["gamma"] = cfg->lookup("model.gamma");
	p["r"] = cfg->lookup("model.r");
	p["tauExt"] = cfg->lookup("model.tauExt");
	p["w0"] = cfg->lookup("model.w0");
	p["epsh"] = cfg->lookup("model.epsh");

	pdim["Ts0"] = p["xs0"] * pdim["DeltaT"] + pdim["T0"];
	pdim["epsT"] = p["alpha"] * pdim["c0"] / pdim["L"];
	pdim["Hm"] = p["eta1"] * pdim["Hs"];
	pdim["h0"] = p["eta2"] * pdim["Hs"];
	pdim["r"] = p["r"] * pdim["c0"] / pdim["L"];
	pdim["b"] = p["gamma"] / pdim["L"] / pdim["tau0"] * pdim["Hm"];
	pdim["tauExt"] = p["tauExt"] * pdim["tau0"];
	pdim["w0"] = p["w0"] / pdim["L"] * pdim["Hm"] * pdim["c0"];
	pdim["epsh"] = p["epsh"] / pdim["L"] * pdim["c0"];
      }
      else {
	std::cout << "Reading model dimensional parameters..." << std::endl;
	pdim["Ts0"] = cfg->lookup("model.Ts0");
	pdim["epsT"] = cfg->lookup("model.epsT");
	pdim["Hm"] = cfg->lookup("model.Hm");
	pdim["h0"] = cfg->lookup("model.h0");
	pdim["r"] = cfg->lookup("model.rd");
	pdim["b"] = cfg->lookup("model.b");
	pdim["tauExt"] = cfg->lookup("model.tauExtd");
	pdim["w0"] = cfg->lookup("model.w0d");
	pdim["epsh"] = cfg->lookup("model.epshd");

	p["alpha"] = pdim["epsT"] * pdim["L"] / pdim["c0"];
	p["eta1"] = pdim["Hm"] / pdim["Hs"];
	p["eta2"] = pdim["h0"] / pdim["Hs"];
	p["r"] = pdim["r"] * pdim["L"] / pdim["c0"];
	p["gamma"] = pdim["b"] * pdim["L"] * pdim["tau0"] / pdim["Hm"];
	p["xs0"] = (pdim["Ts0"] - pdim["T0"]) / pdim["DeltaT"];
	p["tauExt"] = pdim["tauExt"] / pdim["tau0"];
	p["w0"] = pdim["w0"] * pdim["L"] / (pdim["Hm"] * pdim["c0"]);
	p["epsh"] = pdim["epsh"] * pdim["L"] / pdim["c0"];
      }
      p["deltas"] = 1.;
      if (cfg->exists("model.deltas"))
	p["deltas"] = cfg->lookup("model.deltas");
	...
	p["sigmah2"] = p["sigmahInf2"] * 2 * p["epsh"];
      ...
      std::cout << "Ts0 = " << pdim["Ts0"] << std::endl;
      std::cout << "epsT = " << pdim["epsT"] << std::endl;
      std::cout << "Hm = " << pdim["Hm"] << std::endl;
      std::cout << "h0 = " << pdim["h0"] << std::endl;
      std::cout << "r = " << pdim["r"] << std::endl;
      std::cout << "b = " << pdim["b"] << std::endl;
      std::cout << "tauExt = " << pdim["tauExt"] << std::endl;
      std::cout << "w0 = " << pdim["w0"] << std::endl;
      std::cout << "epsh = " << pdim["epsh"] << std::endl;

      std::cout << "Model adimensional parameters: " << std::endl;
      std::cout << "xs0 = " << p["xs0"] << std::endl;
      std::cout << "alpha = " << p["alpha"] << std::endl;
      std::cout << "eta1 = " << p["eta1"] << std::endl;
      std::cout << "eta2 = " << p["eta2"] << std::endl;
      std::cout << "r = " << p["r"] << std::endl;
      std::cout << "gamma = " << p["gamma"] << std::endl;
      std::cout << "tauExt = " << p["tauExt"] << std::endl;
      std::cout << "w0 = " << p["w0"] << std::endl;
      std::cout << "epsh = " << p["epsh"] << std::endl;
